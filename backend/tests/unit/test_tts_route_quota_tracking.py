"""Unit tests for TTS route quota tracking helpers.

Tests the _track_success and _track_quota_error helper functions in
the TTS routes, ensuring that usage is recorded correctly and
...
from src.presentation.api.routes.tts import _track_quota_error, _track_success
...
class TestTrackSuccess:
    ...
    def test_records_request_for_authenticated_user(
        ...
        with patch(
            "src.presentation.api.routes.tts.provider_usage_tracker",
            fresh_tracker,
        ):
            _track_success(user_id, "gemini")

        usage = fresh_tracker.get_usage(str(user_id), "gemini")
    ...
    def test_records_request_for_anonymous_user(self, fresh_tracker: ProviderUsageTracker) -> None:
        ...
        with patch(
            ...
            _track_success(None, "gemini")

        usage = fresh_tracker.get_usage("anonymous", "gemini")
        assert usage.minute_requests == 1
    ...
    def test_different_providers_tracked_separately(
        ...
        with patch(
            ...
            _track_success(user_id, "gemini")
            _track_success(user_id, "gemini")
            _track_success(user_id, "azure")

        gemini = fresh_tracker.get_usage(str(user_id), "gemini")
        azure = fresh_tracker.get_usage(str(user_id), "azure")
...
class TestTrackQuotaError:
    ...
    def test_records_quota_error_in_tracker(
        ...
        with patch(
            ...
            _track_quota_error(user_id, "gemini", exc)
    ...
    def test_enriches_exception_details_with_usage_context(
        ...
        with patch(
            ...
            fresh_tracker.record_request(str(user_id), "gemini")
            fresh_tracker.record_request(str(user_id), "gemini")
            fresh_tracker.record_request(str(user_id), "gemini")
    ...
    def test_enriches_with_anonymous_user(self, fresh_tracker: ProviderUsageTracker) -> None:
        ...
        with patch(
            ...
            _track_quota_error(None, "gemini", exc)
