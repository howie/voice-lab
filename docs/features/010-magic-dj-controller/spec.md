...
- **FR-019**: 系統 MUST 支援音軌拖曳排序，允許 RD 透過拖放調整音軌順序
- **FR-020**: 系統 MUST 支援編輯既有音軌，可透過 TTS 重新生成語音內容
- **FR-021**: 系統 MUST 支援刪除音軌（包含預設音軌和自訂音軌）
...
- **FR-022**: 系統 MUST 將音軌配置（順序、自訂音軌、TTS 生成的音檔）儲存至瀏覽器 localStorage，為 **per-browser** 儲存（每個瀏覽器獨立，非跨裝置同步）
- **FR-023**: 系統 SHOULD 提供音軌配置匯出/匯入功能，允許 RD 在不同裝置間轉移配置
