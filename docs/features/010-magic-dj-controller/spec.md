...
- **FR-007**: 系統 MUST 確保聲音播放延遲小於 100ms（見 SC-003）
...
### DD-001: 預錄模式 UI 佈局 - 四欄排列方式

**決策日期**: 2026-01-29
**選擇**: 方案 B（直向四排 DJ Mixer 風格）

**注意**：UI 佈局分為四個**顯示欄位 (columns)**，但底層音訊模型仍為 3 個播放頻道（Voice / Music / SFX，見 FR-024）。「救場語音」欄位實際使用 Voice 頻道，以 priority:rescue 區分。

**四個顯示欄位**:
1. 救場語音（Voice 頻道，priority: rescue）- 等待填補、緊急結束等
2. 主語音（Voice 頻道，priority: normal）- 各種 TTS 生成的語音
3. 音效（SFX 頻道）- 串場音效，以 mp3 為主
4. 音樂（Music 頻道）- Music generation 產生的背景音樂

**方案 A：橫向四排 (DAW 風格)** - 未採用
...
| 面向 | 方案 A (橫向) | 方案 B (直向) |
|------|--------------|--------------|
| **時間感** | ✅ 強 - 能看到時間軸上的相對位置 | ❌ 弱 - 只看到順序 |
| **操作直覺** | DAW 使用者熟悉 | DJ/Mixer 使用者熟悉 |
| **拖拉距離** | 從右拖到左較遠 | 從右拖到左較近 |
| **同時播放視覺化** | ✅ 容易看出哪些軌道在同時播放 | ❌ 較難看出 |
| **隊列管理** | 需要縮放操作 | ✅ 一目了然的播放順序 |
| **適合場景** | 精確編排、後製 | 現場即興、快速操作 |

**選擇理由**: 方案 B 更適合預錄模式的「現場控制」場景，RD 需要快速從右側 Sound Library 拖拉素材到對應頻道，且隊列管理一目了然。
